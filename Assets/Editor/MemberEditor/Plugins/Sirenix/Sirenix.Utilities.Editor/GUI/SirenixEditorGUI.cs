...
namespace Sirenix.Utilities.Editor
{
    ...
    public static class SirenixEditorGUI
    {
        ...
        public static object ObjectField(object key, Type type, GUIContent label, object value, bool allowSceneObjects = true)
        {
            ...
            var tTitle = value == null ? null : value as IMemberTitle;
            if (EditorGUI.showMixedValue)
            {
                title = new GUIContent("   " + "— Conflict (" + (tTitle != null ? tTitle.TitleName() : type.GetNiceName()) + ")");
            }
            else if (value == null)
            {
                title = new GUIContent("   " + "Null (" + (tTitle != null ? tTitle.TitleName() : type.GetNiceName()) + ")");
            }
            else
            {
                string baseType = value.GetType() == type ? "" : " : " + type.GetNiceName();
                title = new GUIContent("   " + (tTitle != null ? tTitle.TitleName() : value.GetType().GetNiceName()) + baseType);
            }
            ...
            {
                Event.current.Use();
                if (tTitle == null || tTitle.IsClickable())
                {
                    objectPicker.ShowObjectPicker(allowSceneObjects, rect);
                }
                else if(tTitle != null && !tTitle.IsClickable())
                {
                    Debug.Log(tTitle.TitleName());
                }
            }
